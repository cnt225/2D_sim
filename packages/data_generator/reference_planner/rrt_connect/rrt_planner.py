...
try:
    ...
    print("⚠️ OMPL not available. Using Mock OMPL for development...")
    from .mock_ompl import mock_ompl
    ob = mock_ompl.base
    og = mock_ompl.geometric
...
class SE3RRTConnectPlanner:
    ...
    def __init__(self, rigid_body_config: RigidBodyConfig, pointcloud_file: str = None):
        ...
        self.planner_settings = {
            ...
            "range": 0.1,  # RRT-Connect range parameter (default: 0.1 for finer sampling)
            "goal_bias": 0.05,
            "interpolate": True,
            "simplify": True
        }
    ...
    def save_trajectory(self, result: SE3TrajectoryResult, output_file: str, 
                       start_pose: List[float], goal_pose: List[float]):
        """SE(3) trajectory를 JSON 파일로 저장"""
        
        trajectory_data = {
            "trajectory_id": f"traj_rb{self.config.rigid_body_id}_{int(time.time())}",
            "rigid_body": {
                "id": self.config.rigid_body_id,
                "type": self.config.name,
                "semi_major_axis": self.config.semi_major_axis,
                "semi_minor_axis": self.config.semi_minor_axis
            },
            "environment": {
            ...
            "generation_info": {
                ...
                "max_planning_time": self.planner_settings["max_planning_time"],
...
def plan_se3_trajectory_from_poses(rigid_body_id: int, start_pose: List[float], 
                                  ...
                                  max_planning_time: float = 5.0) -> SE3TrajectoryResult:
    """
...
if __name__ == "__main__":
    ...
    try:
        ...
        if result.success:
            ...
            # Save result
            output_file = "test_se3_trajectory.json"
            planner.save_trajectory(result, output_file, start_pose, goal_pose)
        else:
