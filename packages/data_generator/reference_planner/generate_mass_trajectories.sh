...
# 10,000개 환경에 대한 RRT + B-spline 궤적 생성 스크립트

echo "🚀 Starting mass trajectory generation for 10,000 environments..."
...
for i in $(seq -f "%06g" 0 9999); do
    ...
    echo ""
    echo "=== Processing ${env_id} ($((i+1))/10000) ==="
...
" > /dev/null 2>&1
    ...
    if (( (i+1) % 100 == 0 )); then
        ...
        echo "   Processed: $((i+1))/$total_envs ($(( (i+1) * 100 / total_envs ))%)"
        echo "   Success: $success_count, Failed: $failed_count"
        echo "   Success rate: $(( success_count * 100 / (i+1) ))%"
