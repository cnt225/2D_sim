...
# from utils.plot import plot_grasping, plot_grasping_process  # Not used for motion planning
from utils.Lie import *
from utils.ode_solver import get_ode_solver
from utils.utils import init_parameter
# from utils.utils_plotly import PlotlyPlot  # Commented out due to trimesh dependency
...
class MotionRCFM(torch.nn.Module): 
    ...
    def forward(self, current_pose, target_pose, t, v):
        """
        Forward pass for motion planning
        
        Args:
            current_pose: [B, 4, 4] current SE(3) pose
            target_pose: [B, 4, 4] target SE(3) pose (goal)
            t: [B, 1] time parameter [0,1] 
            v: [B, D] environment features from pointcloud
            
        Returns:
            velocity: [B, 6] SE(3) twist vector
        """
        self.count_nfe()
        return self.velocity_field(current_pose, target_pose, t, v)
    
    def train_step(self, data, losses, optimizer):
        """
        Training step for motion planning
        
        Args:
            data: dict with keys:
                - current_T: [B, 4, 4] current poses
                - target_T: [B, 4, 4] target poses  
                - time_t: [B, 1] time parameters
                - pointcloud: [B, N, 3] environment pointclouds
                - T_dot: [B, 6] target velocities
        ...
        # Encode pointcloud - DGCNN expects [B, 3, N] format
        if pointcloud.dim() == 3 and pointcloud.shape[1] < pointcloud.shape[2]:
            # Already in [B, 3, N] format
            v = self.latent_feature(pointcloud)  # [B, 2048]
        else:
            # Convert from [B, N, 3] to [B, 3, N]
            pointcloud_dgcnn = pointcloud.transpose(1, 2)
            v = self.latent_feature(pointcloud_dgcnn)  # [B, 2048]
        
        # Forward pass
        T_dot_pred = self.forward(current_T, target_T, time_t, v)  # [B, 6]
        ...
        for loss_fn in losses:
            ...
            total_loss += loss_val
            loss_dict[f'loss/{loss_fn.__class__.__name__}'] = loss_val.item()
        ...
        # Metrics
        loss_dict['loss/train_loss_'] = total_loss.item()
        loss_dict['metrics/velocity_mse'] = torch.mean((T_dot_pred - T_dot_target)**2).item()
    ...
    def grasp_train_step(self, data, losses, optimizer): # 임의의 x1, 임의의 t에 대해 x(x_t), u(VF), v(model), 이를 바탕으로 backprop (원래 GraspRCFM용)
        x1 = data['Ts_grasp']
        y = data['pc']
